...
class Project_details_model extends MY_Model {
	...
	var $column = array('pd.id', 'pr.name', 'pd.qty'); //set column field database for order and search
    var $order = array('id' => 'asc'); // default order 
	
	protected function _get_datatables_query() {
         
		$this->db->select('pd.id as id, pr.name as name, pd.qty as qty, u.symbol as symbol');
	...
	function get_project_details($id){
		$this->db->select(array('pd.id as id','pd.qty as qty','products_id','p.name','u.symbol'));
	...
	function get_project_details_by_id($id){
		$this->db->select(array('pd.id as id','pd.qty as qty','products_id','p.name as name', 'p.code as code'));
